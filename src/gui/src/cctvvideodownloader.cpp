...
#include "../include/downloadjob.h"
#include "../include/downloadprogresswindow.h"
...
CCTVVideoDownloader::~CCTVVideoDownloader()
{
    ...
    disconnect(&ImageLoader::instance(), nullptr, this, nullptr);
...
void CCTVVideoDownloader::signalConnect()
{
    ...
    connect(&ImageLoader::instance(), &ImageLoader::imageResolved, this, &CCTVVideoDownloader::handlePreviewImageResolved);
...
void CCTVVideoDownloader::isVideoSelected(int r, int c)
{
    ...
    if (!video.image.isEmpty()) {
        m_pendingImageRequestId = ImageLoader::instance().startGetImage(video.image);
    }
